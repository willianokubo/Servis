...
s.nome as servico_nome, s.categoria, s.custo_hora, s.custo_dia, s.custo_semana, u.img_perfil from
usuario_servico us INNER JOIN servico s
on us.id_servico = s.id INNER JOIN usuario u
on us.id_usuario = u.id WHERE s.nome LIKE '%$pesquisar%' and u.email <> '$email'
...
?>
  ...
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="img/img_perfil/<?=$busca["img_perfil"]?>" alt="Avatar" width="50%" class="img-circle">
      <div class="caption">
        <h3 class="text-center"><?=$busca["usuario_nome"]?></h3>
        <p><?=$busca["servico_nome"]?></p>
        <p><a href="venda.php?id=<?=$busca['id']?>" class="btn btn-success btn-block" role="button">detalhes</a></p>
      </div>
    </div>
  </div>
