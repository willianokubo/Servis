...
function formataGabarito(campo, evt) {
    var xPos = PosicaoCursor(campo);
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
    if (!teclaValida(tecla))
    ...
    vr = campo.value = filtraCaracteres(filtraCampo(campo)).toUpperCase();
    if (vr == "A" || vr == "B" || vr == "C" || vr == "D" || vr == "E") {
        campo.value = vr;
    }
    else {
        campo.value = "";
...
function formataMaiusculo(campo, evt) {
    var xPos = PosicaoCursor(campo);
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
    if (!teclaValida(tecla))
...
function formatadoTelefone(campo) {
    //(00) 0000-0000
